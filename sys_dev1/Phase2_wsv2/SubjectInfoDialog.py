    ...
    def edit_workspace(self):
        ...
        try:
            ...
            if editor.exec_() == QDialog.Accepted:
                # 🆕 取得儲存後的新 project_id（可能已被改名）
                saved_project_id = editor.workspace.project_id
                self.load_workspaces()
                self._select_workspace_by_id(saved_project_id)  # 🆕 自動選中
                QMessageBox.information(self, "成功", "Workspace 已更新")
            else:
                # 🆕 取消時也維持原本選中的項目
                original_project_id = workspace.project_id
                self._select_workspace_by_id(original_project_id)
...
class TestConfigEditorDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.color_picker_mode_combo.addItem("12 色調色盤", ColorPickerMode.PALETTE_12.value)  # 🆕
        self.color_picker_mode_combo.addItem("24 色調色盤", ColorPickerMode.PALETTE_24.value)
        self.color_picker_mode_combo.addItem("48 色調色盤", ColorPickerMode.PALETTE_48.value)  # 🆕
    ...
    def save_test_config(self):
        ...
        try:
            ...
            if not self.test_config.drawing_type:
                ...
                return
            # 🆕 unique 檢查：取得 parent（WorkspaceEditorDialog）的 workspace
            parent_editor = self.parent()
            if parent_editor is not None and hasattr(parent_editor, 'workspace'):
                existing_types = [
                    test.drawing_type
                    for test in parent_editor.workspace.drawing_sequence
                    if test is not self.test_config  # 排除自己（編輯時不和自己衝突）
                ]
                if self.test_config.drawing_type in existing_types:
                    QMessageBox.warning(
                        self,
                        "繪畫類型代碼重複",
                        f"繪畫類型代碼「{self.test_config.drawing_type}」已存在於此 Workspace！\n"
                        f"請使用不同的代碼。"
                    )
                    self.drawing_type_edit.setFocus()
                    self.drawing_type_edit.selectAll()
                    return
...
class DrawingTypeDialog(QDialog):
    ...
    def _toggle_artwork(self):
        """🆕 切換展示/關閉繪畫成品"""
        logger = logging.getLogger('DrawingTypeDialog')
        if not self._artwork_showing:
            ...
            # ✅ 只改文字顏色為灰色，背景保持素色
            self.ok_button.setStyleSheet("color: gray;")
            self.ok_button.setEnabled(False)

            self.cancel_button.setStyleSheet("color: gray;")
    ...
    for test in workspace.drawing_sequence:
        ...
        if test.toolbar.color_picker_enabled:
            color_mode_text = {
                ...
                ColorPickerMode.PALETTE_12:    "12 色調色盤",   # 🆕
                ColorPickerMode.PALETTE_24:    "24 色調色盤",
                ColorPickerMode.PALETTE_48:    "48 色調色盤",   # 🆕
